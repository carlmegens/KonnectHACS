...
from custom_components.ouderapp.frontend import (
    CARD_PATH,
    CARD_URL,
...
async def test_interfaces_register_globally_and_keep_panel_hidden(hass, modules):
    ...
    panel = hass.data[frontend.DATA_PANELS]["ouderapp"]
    ...
    assert panel.config["_panel_custom"]["name"] == "ouderapp-panel"
    ...
    assert hass.data[frontend.DATA_PANELS]["ouderapp"] is panel
...
async def test_interfaces_update_only_their_own_module_and_panel(hass, modules):
    ...
    await panel_custom.async_register_panel(hass, "ouderapp", "ouderapp-panel", module_url=old)
    ...
    assert (
        hass.data[frontend.DATA_PANELS]["ouderapp"].config["_panel_custom"]["module_url"]
        == CARD_URL
    )


async def test_interfaces_do_not_replace_an_unrelated_panel(hass, modules, caplog):
    frontend.async_register_built_in_panel(hass, "other", frontend_url_path="ouderapp")
    existing = hass.data[frontend.DATA_PANELS]["ouderapp"]
    await async_register_interfaces(hass)
    assert hass.data[frontend.DATA_PANELS]["ouderapp"] is existing
    assert modules.urls == {CARD_URL}
    assert "OuderApp account panel path is already in use" in caplog.text


async def test_interfaces_tolerate_headless_ha(hass):
    hass.data.pop(frontend.DATA_EXTRA_MODULE_URL, None)
    await async_register_interfaces(hass)
    assert "ouderapp" not in hass.data.get(frontend.DATA_PANELS, {})
...
async def test_interface_failure_logs_no_unrelated_data(hass, modules, caplog):
    with patch.object(
        ...
        await async_register_interfaces(hass)
    assert "OuderApp device interface could not be registered" in caplog.text
...
async def test_resource_failure_is_safe_and_does_not_break_account_setup(hass, caplog):
    ...
    hass.data[LOVELACE_DATA] = SimpleNamespace(resource_mode=MODE_STORAGE, resources=collection)
    await async_register_resource(hass)
    assert "OuderApp card resource could not be registered" in caplog.text
...
async def test_setup_serves_only_bundled_javascript_without_school_data(
    ...
    hass.states.async_set("sensor.ouderapp_synthetic_private", "SYNTHETIC-PRIVATE-SCHOOL-CONTENT")
    with patch("custom_components.ouderapp.frontend.async_at_started") as at_started:
    ...
    assert modules.urls == {CARD_URL}
    assert "ouderapp" in hass.data[frontend.DATA_PANELS]
    client = await hass_client_no_auth()
    response = await client.get(CARD_URL)
    assert response.status == 200
    body = await response.read()
    expected = (
        Path(__file__).resolve().parents[1] / "custom_components/ouderapp/frontend/ouderapp-card.js"
    ).read_bytes()
    ...
    for path in ("/ouderapp_static/manifest.json", "/ouderapp_static/api.py", "/ouderapp_static/"):
