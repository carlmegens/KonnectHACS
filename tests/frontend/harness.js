import '../../custom_components/ouderapp/frontend/ouderapp-card.js';
...
window.fixture = {
  ...
  chatAllowed:true, admin:params.has('admin'), planningPending:false, planningError:false, planningOffline:false, planningTruncated:false, planningEmpty:false, planningStatus:'tentative',
...
const source=['timeline','news','newsletters','messages'].includes(params.get('source'))?params.get('source'):'timeline';
const config={type:'custom:ouderapp-card',config_entry_id:params.get('state')==='choose'?'':'example-account',source,limit:5,show_images:true,...(params.has('room')?{chatroom_id:params.get('room')}: {})};
const mount=document.getElementById('mount');
if(params.get('surface')==='popup') {
  window.surface=document.createElement('more-info-ouderapp');surface.stateObj={entity_id:'sensor.synthetic_ouderapp',attributes:{ouderapp_config_entry_id:'example-account',ouderapp_source:source}};surface.hass=makeHass();
  mount.className='popup';mount.append(surface);window.card=surface._card;
} else if(params.get('surface')==='panel') {
  window.surface=document.createElement('ouderapp-panel');surface.route={path:(params.has('chooseAccount')?'/':'/example-account')+'?source='+source};surface.hass=makeHass();
  mount.className='panel';mount.append(surface);window.card=surface._card;
} else {
  window.card=document.createElement('ouderapp-card');card.setConfig(config);card.hass=makeHass();mount.append(card);
}
if(params.has('editor')) {
  document.getElementById('editor').hidden=false;
  window.editor=document.createElement('ouderapp-card-editor');editor.setConfig(card._config);editor.hass=makeHass();
