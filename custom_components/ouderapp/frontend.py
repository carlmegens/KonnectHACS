...
async def async_register_resource(hass: HomeAssistant) -> None:
    ...
    try:
        ...
        _LOGGER.warning(
            "OuderApp card resource could not be registered; add it in dashboard resources"
        )
...
async def async_register_interfaces(hass: HomeAssistant) -> None:
    ...
    if modules is not None:
        try:
            ...
            existing = hass.data.get(frontend.DATA_PANELS, {}).get("ouderapp")
            if existing is not None:
                config = (existing.config or {}).get("_panel_custom", {})
                if (
                    existing.component_name != "custom"
                    or config.get("name") != "ouderapp-panel"
                    or not _is_own_url(config.get("module_url", ""))
                ):
                    _LOGGER.warning("OuderApp account panel path is already in use")
                    await async_register_resource(hass)
                    return
                if config.get("module_url") != CARD_URL:
                    frontend.async_remove_panel(hass, "ouderapp")
                    existing = None
            if existing is None:
                await panel_custom.async_register_panel(
                    hass,
                    frontend_url_path="ouderapp",
                    webcomponent_name="ouderapp-panel",
                    module_url=CARD_URL,
                )
        except Exception:
            # Never log exception text that may contain unrelated panel data.
            _LOGGER.warning("OuderApp device interface could not be registered")
    await async_register_resource(hass)
...
async def async_setup_frontend(hass: HomeAssistant) -> None:
    await hass.http.async_register_static_paths(
        [
            StaticPathConfig(
                CARD_PATH, str(Path(__file__).parent / "frontend" / "ouderapp-card.js"), False
            )
        ]
    )
