/* OuderApp card 0.1.1 — content stays in this card's memory, never in entity states. */
const STRINGS = {
  nl: {
    ...
    selectOnCard: 'Kiezen in de kaart', openOuderApp: 'Open OuderApp', backIntegration: 'Terug naar de OuderApp-integratie', backDevice: 'Terug naar het apparaat',
    messages: 'Opvangberichten', refresh: 'Vernieuwen', loading: 'Inhoud ophalen…',
    empty: 'Geen items', emptyHint: 'Er zijn geen items voor deze selectie.',
    choose: 'Kies een OuderApp-account', chooseHint: 'Open de kaartinstellingen en selecteer je account.',
    noAccounts: 'Geen toegankelijk OuderApp-account', noAccountsHint: 'Voeg OuderApp toe bij Apparaten en diensten, of vraag de beheerder om toegang.',
    unauthorized: 'Geen toegang tot dit account', unauthorizedHint: 'Vraag de Home Assistant-beheerder om toegang tot dit OuderApp-account.',
    authentication_expired: 'Opnieuw aanmelden nodig', authentication_expiredHint: 'Meld je opnieuw aan bij OuderApp via Apparaten en diensten.',
    ...
    untitled: 'Opvangbericht', retry: 'Opnieuw proberen', noGroups: 'Groepen ophalen…',
  },
  en: {
    timeline: 'Timeline', news: 'News', newsletters: 'Newsletters', conversations: 'Conversations', source: 'Content', conversation: 'Conversation', message: 'Message',
    ...
    selectOnCard: 'Choose in the card', openOuderApp: 'Open OuderApp', backIntegration: 'Back to the OuderApp integration', backDevice: 'Back to the device',
    messages: 'Childcare updates', refresh: 'Refresh', loading: 'Loading content…',
    empty: 'No items', emptyHint: 'There are no items for this selection.',
    choose: 'Choose a OuderApp account', chooseHint: 'Open the card settings and select your account.',
    noAccounts: 'No accessible OuderApp account', noAccountsHint: 'Add OuderApp in Devices & services, or ask your administrator for access.',
    unauthorized: 'No access to this account', unauthorizedHint: 'Ask your Home Assistant administrator for access to this OuderApp account.',
    authentication_expired: 'Sign in again', authentication_expiredHint: 'Sign in to OuderApp again in Devices & services.',
    ...
    untitled: 'Update', retry: 'Try again', noGroups: 'Loading groups…',
  },
};
...
const configValue = (input) => {
  if (!input || (input.type && input.type !== 'custom:ouderapp-card')) throw new Error('Expected custom:ouderapp-card');
  const source = input.source ?? 'timeline';
  if (!['timeline', 'news', 'newsletters', 'messages'].includes(source)) throw new Error('source must be timeline, news, newsletters or messages');
...
class OuderAppCard extends HTMLElement {
  constructor() {
    super();
  ...
  static getStubConfig() { return { type: 'custom:ouderapp-card', limit: 5, show_images: true }; }
  static getConfigElement() { return document.createElement('ouderapp-card-editor'); }
  ...
  async _load(force = false) {
    ...
    try {
      ...
      if (this._config.source === 'messages') {
        ...
        request = { type: 'ouderapp/content', kind: 'messages', config_entry_id: account, conversation: this._roomId, limit: this._config.limit };
      } else {
        request = { type: 'ouderapp/content', kind: this._config.source, config_entry_id: account, limit: this._config.limit };
      }
  ...
  _render() {
    ...
    heading.append(el('h2', '', this._config.title || 'OuderApp'), el('p', 'subtitle', this._config.source === 'messages' ? t.conversations : t[this._config.source]));
    ...
    if (this._loading && !this._data) {
      ...
      this._data.items.forEach((item, index) => {
        ...
        meta.append(el('span', 'group', item.unread === true ? (locale(this._hass).startsWith('nl') ? 'Nieuw' : 'New') : ''));
  ...
  async _loadPhoto(node) {
    ...
    try {
      const response = await this._hass.fetchWithAuth(`/api/ouderapp/${encodeURIComponent(this._config.config_entry_id)}/image/${this._config.source === 'messages' ? 'messages' : 'timeline'}/${encodeURIComponent(id)}`, { signal: controller.signal });
...
class OuderAppCardEditor extends HTMLElement {
  ...
  async _loadAccounts() {
    ...
    try {
      const result = await this._hass.callWS({ type: 'ouderapp/accounts', ...(this._config.source === 'messages' ? { source: 'messages' } : {}) });
      if (epoch !== this._epoch) return;
  ...
  async _loadGroups() {
    ...
    if (!this._hass || !account || this._config.source !== 'messages' || !this._accounts.some((item) => item.config_entry_id === account)) { this._render(); return; }
    this._groupsLoading = true; this._render();
    try {
      const result = await this._hass.callWS(this._config.source === 'messages' ? { type: 'ouderapp/content', kind: 'conversations', config_entry_id: account, limit: 20 } : { type: 'ouderapp/content', kind: this._config.source, config_entry_id: account, limit: 1 });
      if (epoch !== this._epoch) return;
      this._groups = this._config.source === 'messages' ? (Array.isArray(result?.items) ? result.items.slice(0, 50).map((room) => ({ id: room.conversation_id, name: room.title })) : []) : (Array.isArray(result?.groups) ? result.groups : []);
      this._error = null;
    } catch (error) {
      if (epoch !== this._epoch) return;
      this._error = errorCode(error);
  ...
  _change(key, value) {
    ...
    if (key === 'config_entry_id' || key === 'source') { this._config.chatroom_id = ''; this._groups = []; if (key === 'source') this._loadAccounts(); else this._loadGroups(); }
    const config = { ...this._config };
    for (const optional of ['title', 'chatroom_id', 'config_entry_id']) if (!config[optional]) delete config[optional];
  ...
  _render() {
    ...
    const source = select('source', [{ id: 'timeline', name: t.timeline }, { id: 'news', name: t.news }, { id: 'newsletters', name: t.newsletters }, { id: 'messages', name: t.conversations }], t.source); source.firstElementChild.remove(); field('source', t.source, source);
    const account = select('config_entry_id', this._accounts.map((item) => ({ id: safeText(item.config_entry_id, 128), name: safeText(item.title, 160) || 'OuderApp' })), t.choose);
    ...
    group.disabled = this._groupsLoading || !this._config.config_entry_id; if (this._config.source === 'messages') field(selection, t.conversation, group);
    const title = el('input'); title.type = 'text'; title.value = this._config.title; title.placeholder = 'OuderApp'; title.maxLength = 160; title.addEventListener('change', () => this._change('title', title.value)); field('title', t.title, title);
...
const SURFACE_CSS = `${CSS}
  ...
  .tabs { display:flex; overflow-x:auto; border-bottom:1px solid var(--divider-color,#e0e0e0); margin-bottom:20px; }
...
`;
class MoreInfoOuderApp extends HTMLElement {
  ...
  _sync() {
    ...
    if (!this._card) {
      ...
      this._card = el('ouderapp-card'); this._card.setAttribute('embedded', '');
    ...
    const account = safeText(attrs.ouderapp_config_entry_id, 128);
    const source = ['timeline', 'news', 'newsletters', 'messages'].includes(attrs.ouderapp_source) ? attrs.ouderapp_source : 'timeline';
    this._card.setConfig({ type: 'custom:ouderapp-card', config_entry_id: account, source, limit: 20, show_images: true });
    ...
    if (this._linkSignature !== signature) {
      ...
      this._links.replaceChildren(...(account ? [internalLink(words(this._hass).openOuderApp, `/ouderapp/${encodeURIComponent(account)}?source=${source}`)] : []));
    }
  }
}
...
class OuderAppPanel extends HTMLElement {
  ...
  _routeAccount() {
    let value;
    try { value = decodeURIComponent(String(this._route?.path || '').split('?')[0].replace(/^\/ouderapp(?=\/|$)/, '').replace(/^\//, '').replace(/\/$/, '')); } catch { return ''; }
  ...
  async _loadAccounts() {
    ...
    try {
      const result = await this._hass.callWS({ type: 'ouderapp/accounts', ...(this._source === 'messages' ? { source: 'messages' } : {}) });
      if (epoch !== this._epoch) return;
      this._accounts = Array.isArray(result?.accounts) ? result.accounts : [];
  ...
  _renderControls() {
    ...
    const back = internalLink(device ? t.backDevice : t.backIntegration, device ? `/config/devices/device/${encodeURIComponent(device)}` : '/config/integrations/integration/ouderapp');
    ...
    if (!this._routeAccount()) {
      ...
      for (const account of this._accounts) { const option = el('option', '', safeText(account.title, 160) || 'OuderApp'); option.value = safeText(account.config_entry_id, 128); select.append(option); }
